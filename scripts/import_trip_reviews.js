...
async function main() {
  ...
  db.serialize(() => {
    ...
    // Ensure trips table has review_count and trip_reviews table exists
    db.run("ALTER TABLE trips ADD COLUMN review_count INTEGER DEFAULT 0", (e) => {
      // ignore error if column exists
    });
    db.run(`CREATE TABLE IF NOT EXISTS trip_reviews (
    ...
    // Optional: clear existing trip_reviews to avoid duplication
    // db.run('DELETE FROM trip_reviews');
